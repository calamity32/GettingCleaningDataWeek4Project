...
DataNames <- c("sub", "act", as.character(subfeatNames))
...
names(data1) <- gsub("mean", "Mean", humanActivityCols)
names(data1) <- gsub("std", "StandardDeviation", humanActivityCols)

#Creates a second, independent tidy data set with the average of each variable for each activity and each subject.

data2<-aggregate(. ~sub + act, data1, mean)
data2<-data2[order(data2$sub,data2$act),]
